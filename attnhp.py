...
from easy_tpp.model.torch_model.torch_basemodel import JutifiedTorchBaseModel


class SimpleNN(nn.Module):
    def __init__(self, type_dim, time_dim, feat_dim):
        super(SimpleNN, self).__init__()
        self.fc1 = nn.Linear(type_dim + time_dim, feat_dim)
...
class AttNHP(JutifiedTorchBaseModel):
    ...
    def __init__(self, feat_dim, hidden_size, use_ln, time_emb_size, num_layers, num_heads, dropout_rate, num_edge_type):
        super(AttNHP, self).__init__(num_event_types=num_edge_type)
        ...
        self.trans_decoder = SimpleNN(hidden_size, time_emb_size, feat_dim)
    ...
    def loglike_loss(self, time_seqs, time_delta_seqs, type_seqs, batch_non_pad_mask, attention_mask, type_mask, event_feas):
        ...
        enc_out = self.forward(time_seqs, type_seqs, attention_mask, time_seqs)
        return self.trans_decoder(enc_out, event_feas)
        # return enc_out
